...
export function HeroDynamic({ profile }: HeroDynamicProps) {
  // Fallback values if no profile data
  const fullName = profile?.full_name || 'Vishal Singh'
  const bio =
    profile?.bio ||
    'Computer Science student with hands-on MERN stack experience, focused on building scalable, user-centric web applications.'
  const quote = profile?.quote || '"Designing quiet systems that feel warm."'

  // Use getImageUrl to handle fallback to naming convention
  const profilePicture = getImageUrl(profile?.profile_picture_url, 'profile')
  ...
  return (
    <section className="max-w-6xl mx-auto px-6 py-20 flex flex-col md:flex-row items-start md:items-center gap-8 md:gap-12 relative">
      <motion.div
        ...
        className="flex-1">
        <h1 className="text-4xl md:text-6xl font-serif leading-tight text-white">{quote}</h1>
        <p className="mt-4 text-slate-300 max-w-xl">{bio}</p>
      ...
      <div className="relative flex-shrink-0">
        ...
        <motion.div
          ...
          className="absolute -bottom-28 -right-8 md:-bottom-4 md:-right-6 w-48 p-5 bg-yellow-200 rounded-sm shadow-md cursor-pointer border-0 origin-bottom-right">
          {/* Pin hole indicator */}
